...
def audit_jpl_pilot(max_samples: int = SAMPLE_PER_MONTH) -> dict:
    acn = acn_project_dir()
    idx = pd.read_csv(
        acn / "manifests" / "static_file_index.csv",
        dtype={"stationID": str, "file": str},
    )
    mapf = pd.read_csv(acn / "manifests" / "static_api_mapping.csv", dtype={"stationID": str})

    jpl = idx[
        (idx["site"] == "jpl") & (idx["rows"] >= 10) & (idx["gzip_ok"]) & (idx["read_ok"])
    ].copy()
    jpl["month"] = jpl["connection_time"].str[:7]

    matched = mapf[mapf["match_status"] == "matched"]
    matched_jpl = (
        matched[matched["site_static"] == "jpl"] if "site_static" in matched.columns else matched
    )
    m = matched_jpl.copy()
    ...
    for month, grp in jpl[jpl["month"].between("2020-01", "2020-12")].groupby("month"):
        ...
        sample = (
            pilot_files
            if len(pilot_files) <= max_samples
            else pilot_files.sample(max_samples, random_state=SEED)
        )
        pilot_val_rows = 0
        pilot_val_files = 0
        pwr_avail_rows = 0
        ...
        for _, r in sample.iterrows():
            ...
            if len(raw) > 1:
                ...
                dur_min.append(duration.total_seconds() / 60.0)
        ...
        by_month[month] = {
            ...
            "sampled_pilot_row_coverage": round(
                pilot_val_rows / max(int(sample["rows"].sum()), 1), 4
            ),
            "sampled_power_row_coverage": round(
                pwr_avail_rows / max(int(sample["rows"].sum()), 1), 4
            ),
            "matched_sessions": n_matched,
            "sampled_median_duration_min": round(
                float(pd.Series(dur_min).median()) if dur_min else -1.0, 1
            ),
        }
    ...
    result = {
        ...
        "eligibility_rule": (
            "matched_sessions>=300 and sampled_pilot_value_ratio>=0.8 and not abnormal"
        ),
    }
    OUT.mkdir(parents=True, exist_ok=True)
    (OUT / "audit_jpl_pilot_quality.json").write_text(
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            {k: r[k] for k in ("eligible_months", "by_month")},
            ensure_ascii=False,
            indent=2,
        )
    )
