"""E0F-01 运行器：全量输入 manifest + 数据质量审计 + connectionTime 审计 + 四冻结产物。

依据：V2.1 §10；审查结论7 §5；审查结论9。
用法：python experiments/e0_full/run.py [--workers N]
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E0F-01 全量输入审计")
    parser.add_argument("--workers", type=int, default=1, help="扫描并行进程数")
    args = parser.parse_args()

    outputs = run_e0f01(cfg_path=CONFIG, workers=args.workers)
