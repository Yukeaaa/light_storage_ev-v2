...
> retrospective 已冻结 Round 5 启动条件；R5-P0/P0b pre-launch data scouting 初筛和 targeted search
> P1-P4 后，0 个 family 满足 7/7；R5 data acquisition spec 已冻结 P2/P1 最小字段合同。
> 当前不启动 Round 5。
...
| **R5-P0b targeted source search**：聚焦 P2 同源 PV/PCS telemetry 与 P1 transformer constraint telemetry；仍无 single-source 7/7 | targeted data scouting | `reports/core_search/CORE_SEARCH_R5_P0B_TARGETED_SOURCE_SEARCH.md` |
| **R5 data acquisition spec**：冻结 P2/P1 最小字段合同与 schema→semantics→event→effect→7/7 intake gate | data acquisition contract | `reports/core_search/CORE_SEARCH_R5_DATA_ACQUISITION_SPEC.md` |
...
- **当前最新（CORE-SEARCH Round 4 / R4-A0b）**：**core-patent status = NO-GO / 当前无成熟 GO 核心专利**。
  ...
  `reports/core_search/CORE_SEARCH_R5_P0B_TARGETED_SOURCE_SEARCH.md`、
  `reports/core_search/CORE_SEARCH_R5_DATA_ACQUISITION_SPEC.md`。
...
3. **不启动 Round 5**：R5-P0b targeted search 仍无 P2/P1 single-source 7/7；下一步只能按 data acquisition spec 获取同源设备能力约束数据链。
