...
class GapThresholds:
    def __init__(self, p_on_kw: float, delta_r: float, delta_p_kw: float, t_event_min: int,
...
def detect_gap_events(df1m: pd.DataFrame, thr: GapThresholds) -> pd.DataFrame:
    ...
    rows: list[dict] = []
    for _, sess in labeled.groupby("session_id", sort=False):
    ...
    if not rows:
        return pd.DataFrame(
            columns=["session_id", "site", "garage", "station_id", "start_utc", "end_utc", "duration_min",
                     "max_gap_kw", "median_gap_kw", "p95_gap_kw", "gap_energy_kwh", "working_power_median_kw", "month"]
        )
