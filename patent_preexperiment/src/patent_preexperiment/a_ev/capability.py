...
- 更新对象 = **该资源自己的持续能力知识**；
- **更新方向 = 要求方向，更新水平 = 该方向上的观测交付水平**（V0.2：统一符号约定，
  不再对任意情形一律取 `abs(p_meas)`）；
...
def init_capability(spec: ResourceSpec, cfg: dict[str, Any], t0: float = 0.0) -> CapabilityState:
    """初始化（说明书 §10.3 的「无历史」路径：取额定 / 本机限值）。"""
    up = spec.up_limit()
...
def apply_capability_update(
    ...
    """确认「持续可用能力受限」后收缩**要求方向**的能力边界。

    观测水平取 `res.evidence_level`（= 要求方向上的交付分量），
    因此"要求降低出力却降不下来"这类**反向欠交付**也能正确落到对应方向。
    """
    if not res.allows_update:
        return None
    side = "up" if res.evidence_direction == DIR_UP else "down"
    return _shrink(state, obs, side, res.evidence_level, float(cfg["capability"]["conf_step"]))
...
def maybe_recover(
    ...
    # 闸 2：须已越过控制响应观察窗
    if obs.cmd_sent_t > 0.0 and (obs.t - obs.cmd_sent_t) < thr.window_s:
        track.confirm_streak = 0
        return None
...
def writeback_carrier(
    state: CapabilityState, obs: Obs, thr: Thresholds, cfg: dict[str, Any]
) -> dict[str, Any] | None:
    """承接资源执行结果 → 回写**该承接资源自己**的能力状态（权 1【e】）。

    承接失败（在**要求方向上**存在超带欠交付）时收缩其对应方向边界——即为权 7 的
    "维持该承接资源经实际执行结果修正后的能力边界"。
    过执行 / 反向执行不收缩边界（沿用与模块 1 相同的两层语义）。
    """
    band = thr.band(obs.rid)
    dev = deviation_of(obs.p_req, obs.p_meas)
    if not dev.evidences_limitation or dev.deficit < band:
        return None
    side = "up" if dev.req_dir == DIR_UP else "down"
    return _shrink(
        state, obs, side, dev.delivered_in_req_dir, float(cfg["capability"]["conf_step"])
    )
