...
- 图像件只做**扉页级** OCR：台账中公开号/申请人/摘要均为初判（信息可信度低），引用前必须回全文；`*.frontpage.md` 不是全文。台账"证据层级"列区分全文级/扉页级——**扉页级行的"无覆盖"不得解读为全文排除**（01 号报告 §3 口径：当前证据层级下"未观察到已确认连续两环"≠"108 篇全文均不存在"）。另 `tools/ocr_fulltext.py` 支持对单个关键复核对象做全文 OCR（已用于 CN122371331A）。
