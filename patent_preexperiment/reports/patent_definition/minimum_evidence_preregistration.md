...
> 新 P2 完整冻结于 `phase3_p2_preregistration_v1.0.2.md`（8 字段 + Step0 K1/K2/K3 kill
> gates + 机器可审计动作 schema `configs/phase3_p2_action_schema.yaml`；v1.0.1 为
> Review 66 修复版，v1.0.2 为 Review 67 执行级接口 hotfix，均取代 v1.0）。P1 字段 1–8
> 维持 v1.0.2 冻结不变；P3 维持 BLOCKED（本文件 §3 骨架仅作历史保留）。
>
> **冻结效力**：本文档 P1 冻结为 v1.0.2；P2 以 `phase3_p2_preregistration_v1.0.2.md` 为准。
...
> `phase3_p2_preregistration_v1.0.2.md`（D1/D2/D3 机制成立率验证 + Step0 K1/K2/K3 kill
> gates；v1.0.1 修复 Review 66 P0 接口闭环，v1.0.2 封死 Review 67 执行级 hotfix）。
> 本节仅作历史保留，**不得作为执行依据**；新 P2 未出结果前**不启动**。
...
| P2 | CLAIM 1(3/4/5/6)、CLAIM 2/3/4/5 | P-001/P-002/P-003、C-012 | **D1/D2/D3 机制成立率**（v1.1 重定义版，见 `phase3_p2_preregistration_v1.0.2.md`）：K1/K2/K3 全过；M1=M2=1.0、M4=0.0、M3 **natural JPL** recovery trace ≥20/≥5 会话 | K2 失败 → **PROJECT NO-GO**（硬杀线）；K1/K3/M4 失败 → 组合核心缺失 → 极可能 No-Go；Conditional 见新 P2 协议 §6 |
...
- 本文档 = **Phase 3 v1.0.2（P1 冻结）**。**v1.1（2026-08-11，Patent Gate 2 重定义 P2）**：
  P2 骨架被 `phase3_p2_preregistration_v1.0.2.md` 取代（D1/D2/D3 机制验证 + Step0 K1/K2/K3
  kill gates；v1.0.1 = Review 66 接口闭环修复版，v1.0.2 = Review 67 执行级 hotfix）；
  P1 字段 1–8 与 P3 骨架不变。P3 字段 5
  细节展开 = 新版本（v1.2）+ 新测试协议。
