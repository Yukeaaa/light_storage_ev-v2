...
8. 判断所述实际充电响应是否满足**预定边界接触条件**（如实际功率贴近所选边界，连续
   若干周期）；
9. 满足所述边界接触条件时，**改变所述预算修正允许区间**（由保护性模式恢复到更高调整
   权限；功率边界生成方式不变；P2 实现为 M3 段内单向恢复，非双向降级↔恢复）；
...
- **CLAIM 5（强从属，恢复）**：在保护性控制模式下，当检测到实际功率贴近所选边界
  ...
  P2 实现为 M3 段内**单向**恢复，非双向降级↔恢复）。
  （证据层级：P2 jpl_test natural complete recovery traces = 1,060 / 1,060 会话——**机制已观测**；
  "边界接触是否构成恢复更高权限的**有效能力证据**"尚待 **P2.1A falsification**，rolling-Q95
  自相关伪证据风险未排除。M4→M3 是信息驱动转换，不计入 D3 恢复计数。）
...
| 强从属（5，恢复） | P-003（C，机制已观测；trigger 语义有效性待 P2.1A） | natural recovery 1,060 会话，非 replay 凑数；但 rolling-Q95 自相关伪证据风险未排除 |
