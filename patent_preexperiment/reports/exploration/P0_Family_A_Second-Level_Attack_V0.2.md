...
────────────────────────────────────────────────────────────────
Family A（1+7+8）                              → DOWNGRADE（不再作为主候选方向）
```
...
### 5.4 会推翻本判定的条件（留给后续证伪，诚实登记）

本判定基于 S1/S2 两个退化场景。以下任一成立时，H-A5 可复活，但**均须再过 §6 四问**：

- **R1**：存在"同一服务混合支撑、且混合比例随周期改变的方向不可由固定优先级序表达"的**更一般场景**；
- **R2**："被释放的 cond 容量被重新分配给**另一**服务"产生 **义务血缘（lineage）**，
  使义务无法归属到任一服务自身的累计计数器（即 §3.1 的 $\Gamma_j$ 不完备）；
- **R3**：所需控制动作依赖**事件序列**（order-dependent），而不只是当前总量。

> 我的初判：R2/R3 即使成立，也只是把状态从"分解"升级为"血缘台账"，
> 仍需过 Q1（物理资源/控制许可 vs 合同字段）—— 通过概率不高。
> **在拿到 R1/R2/R3 的具体反例前，不建议为它继续投入检索与仿真资源。**
...
**推荐（本轮倾向）**：**Family A 收线，转 Family B 轻量 scout。**

```text
Family A（1+7+8）  → DOWNGRADE
                     保留为 Problem-Reality 记录，不再投入检索/仿真资源
                     除非出现 §5.4 的 R1/R2/R3 具体反例

Family B（6）      → 只做轻量 scout：
                     同一 PCS/BESS/EVSE 的 S_rated / I_rated
                     在有功服务与无功/谐波/不平衡治理之间的竞争，
                     商业系统与专利已做到什么程度
                     若最终只是 P²+Q² ≤ S² 或常规 OPF → 直接降级
                     ❗ 不得因 Family B 分散资源，除非它先给出超出常规 P-Q 约束的结构缺口

数据线（3-b）      → 不扩张。只有某个机制先通过 G1–G5，才去问"哪个公开数据能验证它"
专利候选           → 仍无编号。本轮不注册任何候选
入口               → 尚不建立。按用户口径，入口须指向"最新状态"；
                     但本轮结论为 DOWNGRADE，入口的写法与时机建议在下一轮一并确定（见 §11）
```

**为什么不顺手建 AGENTS.md 入口**：本轮结论是"Family A 降级"而非"进入机制化"，
若现在按原方案写 `residual hypothesis under test`，会与 V0.2 的实际结论相反。
建议入口延后到 Family B scout 结果出来后一次写清（Family A = DOWNGRADED / Family B = SCOUT 中），
避免入口出生即过期。
