...
A 说明书骨架        = 待用户复核 16 号判 PASS 后展开（尚未开始）
```
...
> **说明书义务三条（A 骨架必须落实）**：①【b】的五类可实施判据；② **资源级执行偏差 → 站级功率缺口的等效换算与方向聚合**（统一功率符号约定 + 预设站级功率参考点 + 按方向聚合）；③ **其他可控资源"当前能力状态"的初始化**（额定能力 / 本地限值 / 最近一次已确认边界 / 预设保守值中的至少一种，此后依执行证据持续更新）。三条**均不进独立权**。
...
- **A 说明书骨架的前置** = 用户复核 `patent_definition/16_A权利要求语言v2定稿_反馈对象闭合.md` 判 PASS。骨架须按 16 号 §7 义务清单落定义位（【b】五类可实施判据 + **资源级偏差→站级缺口的等效换算与方向聚合** + **其他可控资源"当前能力状态"的初始化**），背景技术按 10 号 §4 引用位。
- **不再开第三轮 A 权项架构讨论** —— 14 号（架构收口）+ 15 号（Claim 语言收尾）+ 16 号（反馈对象闭合）已覆盖全部 P1/P2 与 Open items 1–5。
