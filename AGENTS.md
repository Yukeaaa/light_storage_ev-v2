...
线 1（专利文本）：A 说明书全文 V1 ✅ 已出（18 号）→ 下一步 = A+C 第二轮并排校验
...
**线 1（专利文本）**：**A 说明书全文 V1 已出（18 号）** → 下一步 = **A+C 第二轮并排校验**（两案全文均已就绪：C = `C_控制资格状态机_说明书全文_V1.md`，A = `18_A说明书全文_V1.md`）。

- 第二轮校验重点看**三类污染**：① 共享输入词是否被误写成相同机制（尤其"通信"一词）；② 组合实施方式（A §15 / C §14）是否偷偷变成必要特征；③ A 的"事务级承接排除"与 C 的"持久资格准入"是否在全文中重新混起来。
- 校验完成前**不做无谓润色**（18 号已按骨架一次性展开；修订应来自评审意见而非自我打磨）。
