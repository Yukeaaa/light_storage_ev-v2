...
**A 侧 —— A Claim 反审 / 语言收口阶段 CLOSED；说明书全文 V1 已出**：

```text
16 号              = 最终权威稿（最后一个对象归属漏洞，非第三轮架构讨论）：
                    ① 权 1【e】明确反馈对象——"所述承接资源"执行修正后的实际执行结果 → 回写"对应承接资源"的
                       方向化可用能力状态（修正前归属不明，字面可错读为用承接资源的执行结果去更新偏差资源的状态）
                    ② 权 12 执行反馈单元 + 能力状态维护单元同步镜像
                    ③ 说明书义务新增：其他可控资源"当前能力状态"如何初始化

A Claim v2 架构     = PASS ｜ 权 1 = PASS ｜ 权 2–11 = PASS ｜ 权 12 = PASS
A Claim v2          = PASS
A Claim 阶段        = CLOSED（2026-09-10）
17 号              = A 说明书骨架 V1（已出；**PASS WITH EXPERIMENT PLAN** —— 5 项起草阻断/顺手修已就地修订：
                    ①能力边界恢复"至少一个方向"母概念 ②§6 局部运行条件改双向归因（不再一律排除）
                    ③§14 改为"基于公开运行序列的离线处理示例"并严守证据边界 ④§8 站级约束移到承接层
                    ⑤§11 删除-C 强检验只留骨架/内部层，正式正文改正面表述）
编号体系           = 冻结：权 1 方法独立权 ｜ 权 2–11 方法从属权 ｜ 权 12 系统独立权（十级退守位不压缩）
18 号              = A 说明书全文 V1（已出；§1–§16 + 附录 A 内部证据档 + 附录 B 16 项护栏自检）：
                    ① 背景技术四段降强式写法（内部状态降额 / 执行结果回写计划 / 模型预测差异 / 第一最近现有技术）
                       + 不足归纳；不写 claim chart、不写对整个领域的绝对结论
                    ② §6 五类归因判据表（判定内容 / 实现方式 / 典型判定作用），资源局部运行条件按类型与持续性双向归因
                    ③ §8 等效换算与方向聚合（符号约定 / 参考点换算 / 方向聚合 / 与能力状态分工 / 与承接层分工）
                    ④ §10.3 能力状态初始化（有历史 / 无历史两路径）
                    ⑤ §14 严格两层切分（真实观测层 / 设计演示层）+ 界限声明
                    ⑥ 有益效果全为功能性表述，**无任何数字位**（不预留"待填数字"空洞）
                    ⑦ 附录 A 记录 A 证据状态（A-M3–M6 UNVALIDATED、RD-1/RD-2 STOP、不进入专利证据链）

── A 侧双线并行（2026-09-10 起）──
线 1（专利文本）：A 说明书全文 V1 ✅ 已出（18 号）
                    → A+C 第 2 轮全文并排校验 ✅ = **PASS WITH WORDING FIXES**（`reports/patent_pool/A+C_第2轮全文并排校验_V1.md`；3 处 wording 级已就地修正，0 处实质污染）
                    → 第一波申请包整理 ✅ 工作单与核验 V1 已出（`reports/patent_pool/第一波申请包整理_工作单与核验_V1.md`）
                      · claim-support matrix：A 全通过；C 权 9 支持缺口**已就地补强**（C §5 术语定义）
                      · 摘要正文（两案）已出；附图规范已出（绘制留定稿）
                      · prior-art 提交级核验清单：A 可点名 CN122203352A；CN122371331A 与 C 的 lit45 **建议不点名**
                      · 跨案提交纪律 6 条已锁；去内部信息清扫剥离清单 + 基线数据已出（治理词全部落在待剥离层，零混入正文）
                      · HIL 实现变体上位支持审查：**无缺口**
                    → 第一波正式提交源文件 ✅ 已装配并通过**机械验收（5 项）+ 人工语义验收**（`reports/patent_filing_wave1/`）
                      · 每份只含四部分：权利要求书 + 说明书 + 说明书摘要 + 说明书附图
                      · A_正式提交源文件_V1.md（601 行）／C_正式提交源文件_V1.md（403 行）
                      · 机械验收（`reports/patent_pool/第一波提交源文件验收_V1.md`，工具 `tools/filing_wave1_verify.py`）：
                        ①权项零漂移（A 12/12、C 10/10 归一化后与权威稿一致）②结构完整（八项齐备）
                        ③内部信息残留 25 项词表全 0 ④内部交叉引用/评审语言残留为 0 ⑤摘要字符数合规（A=275、C=263 ≤300）
                        另：交叉术语无越界
                      · 人工语义验收另修 3 项：A §15 去内部删除强检验痕迹；C 实施例四伪代码消除逻辑不可达分支；两案摘要压缩定稿
                      · 状态口径：**技术内容权威源 = 冻结；法律 / 格式定稿 = 待代理人**
                      · 待确认项：**0**（C 判定链「非运行工况 + 一般告警 → 第三等级·仅观测」已于 2026-09-11 确认保持，理由 = 第二等级要求"非空受限控制动作"，非运行工况缺此基础）
                      · **冻结口径**：两案唯一技术源 PASS ｜ 权项保真 PASS ｜ 说明书人工语义验收 PASS ｜ 摘要形式要求 PASS ｜ 附图绘制输入 READY → **第一波技术文本正式交代理人**
                    → 下一步 = 交代理人法律语言与格式加工（附图绘制、段落编号）；绘图委托须含工作单 §3.2.1 的四条硬约束
                    ⚠️ **后续不做主动润色**。除代理人提出明确法律 / 格式意见外，任何涉及**状态对象、归因门、资格等级、恢复语义或跨案边界**的修改，均按**实质修改**退回审查（例：合并 A 的 capability 与 C 的 qualification / 删除归因门 / 把 C 恢复资格写成能力恢复）
线 2（有效性证据）：A_有效性验证实验设计_V0.1.md（A-EV 计划）
                    三层证据：公开数据模块验证 → HIL 可控真值闭环 → 真实站 shadow / 受控闭环
                    ...
                    → **A-EV V0.3 工程架构修订 ✅（2026-09-12）——四个 HIL 前工程阻断项全部软件侧关闭**（`reports/patent_definition/A_工程架构与设备适配层_V0.3.md`；评审定位 = "真实站可运行"工程落地性复审）
                      · 评审 9 项 pre-freeze 要求全部落实并复核 PASS：signed deviation 全链统一（e=P_req−P_meas 贯穿归因→缺口→能力更新→回写）｜ LimitDir 拆 UNKNOWN/BOTH、未知不作正向证据 ｜ RESID_full+steady 双指标（含 PCC）｜ response_delay 真实生效 ｜ 阈值产生规则先冻结（per-resource 带 = max(分辨率,k·σ,α·P_rated)）｜ 三场景集分离 ｜ 主承接改确定性 contribution ｜ A-no-state / A-no-writeback 消融 ｜ 稳健性矩阵预注册（方向×幅值×资源×repeats=5×seed=101）
                      ...
                      · **S9 跨事务复用已入正式评价集**：T1 限值收缩 R0（up_bound 100→30）→ 事务清空但限值未解除 → T2 新缺口下 A 经 R2 区间模型减吸收承接（bad_cmd=0）vs A_no_state 二次不可执行命令（bad_cmd=0.19）——权 7 事务级排除 ≠ 权 1【c】/【e】持久状态取得可运行实验区分
                      · **阻断项 1 ✅ 动态可行功率区间**：`ResourceSpec` 改 `P ∈ [p_min, p_max]`（kind: bess/pv/evse）+ 运行点 headroom（减吸收=上向），`bidirectional` 降为兼容别名；SOC 约束仅 bess——单向充电桩/PV 成为区间模型的一等公民，权项语言不变
                      · **阻断项 2 ✅ 命令链四层留痕**：`Obs` 增 requested/accepted/reported_limit_active；上报型本地限值（accepted<requested）不构成执行偏差证据（判据 4b' REPORTED_LIMIT）且按 Reported 包络封顶 headroom；S10 演示（A 不收缩、B0 误收缩）
                      · **阻断项 3 ✅ 双通道解耦**：PCC 独立表计观测行 + 模块 6 监督后备（`A_sup`，工程层默认关）——S11（通信冻结+真实未执行并存）：A/A_sup 均 EMUR=0，A_sup 站级残差 7.58→1.21 而能力知识不被污染；B0 靠错误更新才补上；监督层与归因通道共用权 7 写回确认条件（显性失败排除 + 防抖 + 上报包络三条工程纪律落入代码）
                      · **阻断项 4 ✅（规范）**：设备适配层与控制权边界成文——BESS/PV/EVSE 字段合同（必读/必写/周期/超时/clamp/ack/fallback）、监督层定位与七条硬边界、四层包络装配（Physical∩Reported∩Learned∩Service）、多速率响应窗、首套台架 = 2 PCS + PCC 表 + EMS；V2G 不作第一轮依赖
                      · 三场景集留档：evaluation（12 场景×7 策略，新增 S10/S11/A_sup）/ robustness（48 条件）/ commissioning（p95=12s→建议窗 15s，数值仍属候选）
                      · 质量门：pytest 全过（a_ev 43 项）、a_ev 包 ruff 0 错、mypy strict 11 文件 0 错；结果 config_hash 与配置一致
                      · 下一步：HIL adapter 实现（BESS-PCS 先行，按 V0.3 §5 规范）→ HIL commissioning（per-resource 响应窗落定）→ `--freeze` 落 V1.0 数值并加锁 → formal A-EV
                    → **A-EV HIL 数据合同与准入门 V0.1 ✅（2026-09-12 第二轮复审后）——synthetic 算法阶段封板**（`reports/patent_definition/A_HIL数据合同与准入门_V0.1.md`）
                      · 可机器执行的准入检查：`a_ev/hil_contract.py` + `configs/a_ev_hil_contract.yaml`——HIL 第一阶段强制字段清单（command_id/requested/translated/accepted/write_ack/control_owner/override_source/override_reason/applied_timestamp/P_meas/PCC + 约束类字段）逐一声明责任域与支撑机制；`admit()` 输出逐机制 VERIFIABLE/DERIVED/NOT_VERIFIABLE + 整体 ADMITTED/CONDITIONAL/REJECTED
                      · 判定规则预注册：缺层 ⇒ 对应机制显式标记不可验证（不事后猜）；推导口径 ⇒ CONDITIONAL（commissioning 实测验证推导链后升级）；时钟超预算 / 控制权多主 / 核心（requested、p_meas）缺失 ⇒ REJECTED
                      · 当前登记口径 = **CONDITIONAL**：accepted_setpoint 走推导（BMS 限值 + PCS 限功率寄存器 + 控制日志），commissioning 期间实测验证
                      · REPORTED_LIMIT 边界冻结为设计决定：BMS 上报限值只封 Reported 包络，**不写 ExecutionLearned**（"设备自报能做到多少" ≠ "执行证据证明能做到多少"，两层独立保留）
                      · 定级（评审第二轮）：架构 PASS / 真实设备接入 CONDITIONAL PASS——真实 PCS 通讯/控制未验证、HIL 闭环未开始正式验证、真实站效果 UNVALIDATED；**不再改 synthetic 算法（封板），边际收益全部转入 HIL adapter**
```

> **A 证据状态（须与上述双线区分）**：A-M3–M6 = **UNVALIDATED**。说明书全文只写**可实施机制**，不写效果；实验通过其自身门槛前，结果**不得**进入任何申请文本。

> **权威文件**：A 权利要求语言以 `reports/patent_definition/16_A权利要求语言v2定稿_反馈对象闭合.md` 为准（**含完整权项集，自足**）；15 号、14 号（及 12 号 §4/§5）仅作历史记录。14 号 §1/§5/§6/§7/§8 与 15 号 §1/§2【a】–【d】/§3/§5/§6/§7 **继续有效**。
> **注意**：A 方案权 1 文本已相对 12 号 §4.1 修订，故**不能沿用"独立权字节级未变"的不变量**。14 号 §8、15 号 §6、16 号 §6 已对冻结证据集做**解析性重比对 / delta**（未新开检索），判定不变。若后续新增独立权项（权 13+），仍按治理规则 = 新版本 + 重开检索判定。
> **说明书义务三条（A 全文必须落实）**：①【b】的五类可实施判据；② **资源级执行偏差 → 站级功率缺口的等效换算与方向聚合**（统一功率符号约定 + 预设站级功率参考点 + 按方向聚合）；③ **其他可控资源"当前能力状态"的初始化**（额定能力 / 本地限值 / 最近一次已确认边界 / 预设保守值中的至少一种，此后依执行证据持续更新）。三条**均不进独立权**。
> **A 侧语义已稳定（对 A/C 隔离有利）**：权 3 / 权 6 / 权 7 共同确立「**本轮 / 当前控制事务中的承接选择 ≠ 设备持久控制参与资格**」。
...
**线 1（专利文本）**：**A 说明书全文 V1 已出（18 号）→ A+C 第 2 轮全文并排校验 = PASS WITH WORDING FIXES（已完成，2026-09-11）**。

- 审计报告 = `reports/patent_pool/A+C_第2轮全文并排校验_V1.md`：六维度对照 + 五个高风险词（通信/异常/运行/资源/状态）逐句审计 + 三类污染逐条命中检查 + **三层删除强检验**（删除 A §15 / 删除 C §14 / 去除对方案接口词后权 1 链完整性）+ 技术效果对照。
- 结论：**0 处实质污染、0 项一票否决信号命中**；5 处 wording 级发现 → **3 处已就地修正（A §6.1判据4/§6.4、A §15、C §14；均不涉权项语言，故未新建 Claim 版本）**，1 处观察保留（A §15"可控资源范围"）。
- **下一步 = 第一波 A/C 文本进入代理人 / 最终申请整理阶段。**
- 修订应来自评审意见而非自我打磨（本轮即按此执行：**不做无谓润色**，只修审计命中项）。
...
**线 2（有效性证据）**：按 `patent_definition/A_有效性验证实验设计_V0.1.md` 推进；**A-EV：V0.2 PRE-FREEZE CLOSED → V0.3 工程架构修订 → HIL 准入门落地（2026-09-12），synthetic 算法阶段封板**，现状权威 = `patent_definition/A_HIL数据合同与准入门_V0.1.md`。

- 软件侧已完成：算法参考实现（signed deviation 全链 / 消融 / S9）、动态可行功率区间（[Pmin,Pmax]）、命令链四层留痕（requested/accepted/reported）、能力证据与 PCC 站级残差双通道解耦（模块 6 监督后备，工程层）、阈值两步冻结闭环（formal 强制验锁、无标量回落）、设备适配层与控制权边界规范、**可机器执行的 HIL 数据合同与准入门**。
- 执行顺序（剩余）：**字段可得性现场核对**（`admit()` 从预登记口径替换为实测口径；accepted 推导链在 commissioning 实测验证）→ BESS-PCS adapter 实现（按 V0.3 §5 规范 + 合同字段清单）→ 首套台架五问（2 PCS + PCC 表 + EMS）→ HIL commissioning 标定（per-resource 响应窗落定）→ 准入门升级 ADMITTED → V1.0 数值冻结（`--freeze` + hash）→ formal A-EV（A-EV-1 负对照 → A-EV-1~5）→ 真实站 shadow → 受控闭环。
...
reports/patent_definition/16_A权利要求语言v2定稿_反馈对象闭合.md       # A 权利要求语言权威稿（自足：权 1 / 权 2–11 / 权 12）
...
reports/patent_definition/A_HIL数据合同与准入门_V0.1.md              # A-EV 现状权威：HIL 准入门（可机器执行）+ synthetic 封板 + 定级表
reports/patent_definition/A_工程架构与设备适配层_V0.3.md             # 四工程阻断项关闭 + 适配层/控制权边界规范
reports/patent_definition/A_算法原型与可控真值场景台_V0.2.md          # V0.2 pre-freeze CLOSED（评审 9 项对照 + 锁闭环 + S9）
reports/patent_definition/A_算法原型与可控真值场景台_V0.1.md          # V0.1 说明（历史，已被 V0.2/V0.3 取代为现状）
...
reports/patent_pool/A+C_第2轮全文并排校验_V1.md                 # 全文级隔离审计（PASS WITH WORDING FIXES）
reports/patent_pool/C_控制资格状态机_说明书全文_V1.md             # C 说明书全文（附录 A 为内部证据档，不进正式文本）
reports/patent_filing_wave1/A_正式提交源文件_V1.md                # 【交代理人】A 唯一技术源（权项+说明书+摘要+附图清单）
reports/patent_filing_wave1/C_正式提交源文件_V1.md                # 【交代理人】C 唯一技术源
reports/patent_pool/第一波提交源文件验收_V1.md                    # 提交源文件验收（权项零漂移/结构完整/内部信息为 0）
```
...
- **改版后必做**：diff 独立权文本。
  - **若字节级未变** → 既有的 prior-art 压力测试结论**继续有效、不重跑**，并在新版文件中写明该不变量。
  - **若已变** → **不能**沿用上述不变量。处理方向：对**冻结证据集**做**解析性重比对 / delta**（不新开检索）；逐项判断变更性质（**因果修正 / 枚举泛化 / 对象归属澄清**）；标注为**解析性（非条款级）**；**若新增了独立权项** → 新版本 + **重开检索判定**（解析性重比对不足以覆盖）。
