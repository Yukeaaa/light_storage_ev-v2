...
**A 侧 —— 权利要求语言 v2 定稿（待复核）**：

```text
12 §4/§5 评审      = A Claim v2 架构 PASS ｜ 权 1 主机制 PASS ｜ 权 12 系统架构 PASS ｜ 从属梯结构 PASS
                    （一度判 A 独立权 v2 架构 CONDITIONAL PASS、§5 从属梯 PASS WITH FIXES）
14 号              = 第一轮收口：2 个 P1（【d】缺口因果倒置；权 10 编号硬冲突）+ 3 个 P2（【b】归因门上位化、
                    【e】对象模型、系统权功能单元镜像）；Open items 1–5 全部关闭
15 号              = 权利要求语言 v2 定稿（Claim 语言收尾，非架构反审）：
                    ① 权 1【d】明确"其他可控资源各自当前的方向化可用能力状态"（对象归属澄清；权 12 镜像）
                    ② 权 8 消除"站级功率控制要求/站级目标功率"对象跳变，改为直接调整权 1 的"功率控制要求"
                    ③ 权 2 明确 per-resource state ≠ per-class state
                    ④ 权 3/权 7（+ 权 6 一致性）改事务级选择排除语言，与 C 的持久资格/资源准入划清边界
编号体系           = 冻结：权 1 方法独立权 ｜ 权 2–11 方法从属权 ｜ 权 12 系统独立权（十级退守位不压缩）
A 说明书骨架        = 待用户复核 15 号判 PASS 后展开（尚未开始）
```

> **权威文件**：A 权利要求语言以 `reports/patent_definition/15_A权利要求语言v2定稿.md` 为准；14 号（及 12 号 §4/§5）仅作历史记录。14 号的 §1 收口对照表、§5 编号体系、§6 Open items 关闭记录、§7 说明书起草义务、§8 解析性重比对**继续有效**。
> **注意**：A 方案权 1 文本已相对 12 号 §4.1 修订，故**不能沿用"独立权字节级未变"的不变量**。14 号 §8 + 15 号 §6 已对冻结证据集做**解析性重比对 / delta**（未新开检索），判定不变。若后续新增独立权项（权 13+），仍按治理规则 = 新版本 + 重开检索判定。
> **说明书义务两条（A 骨架必须落实）**：①【b】的五类可实施判据；② **资源级执行偏差 → 站级功率缺口的等效换算与方向聚合**（统一功率符号约定 + 预设站级功率参考点 + 按方向聚合；不进独立权）。
...
- **A 说明书骨架的前置** = 用户复核 `patent_definition/15_A权利要求语言v2定稿.md` 判 PASS。骨架须按 15 号 §7 义务清单落定义位（含【b】五类可实施判据 + **资源级偏差→站级缺口的等效换算与方向聚合**），背景技术按 10 号 §4 引用位。
- **不再开第三轮 A 权项架构讨论** —— 14 号（架构收口）+ 15 号（Claim 语言收尾）已覆盖全部 P1/P2 与 Open items 1–5。
...
reports/patent_definition/15_A权利要求语言v2定稿.md                # A 权利要求语言权威稿（权 1 / 权 2–11 / 权 12）
