...
- 治理双轨（同见 04 文件 §7）：**A 轨** = 核心专利证据轨，R5 7/7 唯一准入，Round 5 NOT STARTED；**B 轨** = 机制发现轨，独立编号 RD-*（不挂 R5），允许真实公开数据/benchmark/标注仿真，只产出机制发现与方向淘汰，禁闭环收益结论。首发 RD-1（M5BAT 多单元响应异质性）任务卡在 `patent_preexperiment/reports/research_discovery/`：4 个 LMO 单元（非 10 单元）、无温度/限值字段（C 特征集仅 SOC+近期响应）、test_1 不可对齐，阶段门 A0–A5 与基线 ladder（B0/B1/B2 vs C）已冻结，C ≤ B2 即 STOP。
