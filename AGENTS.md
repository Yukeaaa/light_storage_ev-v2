...
线 1（专利文本）：A 说明书全文 V1 ✅ 已出（18 号）
                    ...
                    → 第一波申请包整理 ✅ 工作单与核验 V1 已出（`reports/patent_pool/第一波申请包整理_工作单与核验_V1.md`）
                      ...
                      · HIL 实现变体上位支持审查：**无缺口**
                    → 第一波正式提交源文件 ✅ 已装配并通过机械验收（`reports/patent_filing_wave1/`）
                      · 每份只含四部分：权利要求书 + 说明书 + 说明书摘要 + 说明书附图
                      · A_正式提交源文件_V1.md（601 行）／C_正式提交源文件_V1.md（409 行）
                      · 验收（`reports/patent_pool/第一波提交源文件验收_V1.md`，工具 `tools/filing_wave1_verify.py`）：
                        权项零漂移（A 12/12、C 10/10 归一化后与权威稿一致）／结构完整（八项齐备）／内部信息残留 25 项词表**全 0**／交叉术语无越界
                      · 状态口径：**技术内容权威源 = 冻结；法律 / 格式定稿 = 待代理人**
                      · 待处理形式项：**摘要字数**（A=352、C=395，受理通常 ≤300）——按"沿用已通过版本"未自行压缩
                    → 下一步 = 交代理人法律语言与格式加工（附图绘制、段落编号、摘要字数）
