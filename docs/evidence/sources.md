...
| S-007 | P2 formal frozen outcome（phase3_p2_preregistration_v1.0.2 §6）+ R1_expansion_audit#最终判定（P2 冻结判门：SUCCESS / NARROW GO；M1=1.0、M2=1.0、M4=0.0、M3 natural 1,060 会话；evidence freeze 判门，非重算） | `results/raw/phase3_p2/P2_patent_gate.md`、`results/raw/phase3_p2/p2_test_summary.json`（sentinel consumed，rerun PERMANENTLY PROHIBITED） | P-001、P-002、P-003 |
| S-008 | phase3_p2_preregistration_v1.0.2 §5.7 M2/K2 + P2 formal frozen outcome（D2 权限等级编码为数值 action set → accept/clip；m2_cov=0.376743 实际生效；n_diff_prot_normal=72,067） | `reports/patent_definition/phase3_p2_preregistration_v1.0.2.md` §5.7 + `results/raw/phase3_p2/p2_test_summary.json` | P-002 |
| S-009 | phase3_p2_preregistration_v1.0.2 §5.6-5.7 + P2 formal frozen outcome（D3 recovery trace 计数口径：M4→M3 为信息驱动转换，不计入 D3 恢复；jpl_test M4=0.0、M3 natural complete=1,060） | `reports/patent_definition/phase3_p2_preregistration_v1.0.2.md` §5.6-5.7 + `results/raw/phase3_p2/p2_test_summary.json` | P-003 |
